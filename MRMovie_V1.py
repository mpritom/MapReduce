...
class MRMovie(MRJob)
    def steps(self):
        ...
        MRStep(mapper=self.reducer_create_pairs),
        MRStep(mapper=self.reducer_compute_similarities),
        MRStep(mapper=self.reducer_output),
        MRStep(mapper=self.reducer_sort_results)]
    
    def configure_options(self):
        super(MRMovie, self).configure_options()
        self.add_passthrough_option(
            '-l', '--lowerbound', action="store", type='float', default=0.4, help='lower bound of similarity.')
        self.add_passthrough_option(
            '-p', '--minpairs', action="store", type='int', default=8, help='Minimum number of rating pairs.')
        self.add_passthrough_option(
            '-m', '--moviename', action="append", default=[], help='Movie name to look up for similar movies')
        self.add_passthrough_option(
            '-k', '--numofitems', action="store", type='int', default=15, help='Number of similar movies to show.')
